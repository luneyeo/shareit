 ...
 *
 * 필드 구성·유효성·레이아웃·제출 푸터를 한곳에 모으고, 등록/수정 차이는
 * `defaultValues`·`submitLabel`·`onSubmit` 주입만으로 처리합니다. 상태는
 * react-hook-form으로 관리하며, 비네이티브 필드(이미지·카테고리·태그)는 각
 * `*Field`가 `Controller`로 감싼 controlled 컴포넌트입니다.
 *
 * @example
 * ```tsx
 * // 등록
